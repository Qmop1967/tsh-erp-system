...
export function ItemsPageEnhanced() {
  ...
  // Fetch data with branch awareness
  const fetchItems = async () => {
    try {
      setLoading(true)
      const params = getBranchParams({ search: searchTerm })
      
      // Try the new API first, fallback to migration API
      try {
        const response = await itemsApi.getItems(params)
        setItems(response.data.data as any[] || [])
        console.log('✅ Fetched items from main API:', response.data.data?.length || 0)
      } catch {
        // Fallback to migration API
        const response = await fetch(`/api/migration/items/?${new URLSearchParams(params).toString()}`)
        if (response.ok) {
          const data = await response.json()
          setItems(data)
          console.log('✅ Fetched items from migration API:', data.length)
        } else {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/migration/categories/')
      if (response.ok) {
        const data = await response.json()
        setCategories(data)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingItem 
        ? `/api/migration/items/${editingItem.id}`
        : '/api/migration/items/'
  ...
  const handleDelete = async (item: Item) => {
    ...
    try {
      const response = await fetch(`/api/migration/items/${item.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
...
// Import and re-export the dashboard as the default component
import { ItemsDashboard } from './ItemsDashboard'

// Export the dashboard as the default (what users see when clicking "Items" in sidebar)
export const ItemsPage = ItemsDashboard

// Keep the full items management page available as a named export
export { ItemsPageEnhanced as ItemsManagement }
