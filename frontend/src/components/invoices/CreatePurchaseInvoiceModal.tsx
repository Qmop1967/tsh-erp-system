...
import { Vendor, Item, InvoiceStatusEnum, InvoiceTypeEnum, PaymentTermsEnum } from '../../types';
import { vendorsApi, itemsApi, invoiceApi } from '../../lib/api';
...
const CreatePurchaseInvoiceModal: React.FC<CreatePurchaseInvoiceModalProps> = ({
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    // If product is selected, auto-fill unit cost
    if (field === 'product_id' && value) {
      const selectedProduct = products.find(p => p.id === parseInt(value));
      if (selectedProduct) {
        updatedItems[index].unit_cost = selectedProduct.costPriceUsd;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const invoiceData = {
        ...
        invoice_type: InvoiceTypeEnum.PURCHASE,
      ...
      await invoiceApi.createPurchaseInvoice(invoiceData);
      addNotification({
        type: 'success',
      ...
      addNotification({
        ...
        message: 'Failed to create purchase invoice. Please try again.',
      });
    } finally {
      setLoading(false);
    }
