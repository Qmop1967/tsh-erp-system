...
class CustomerFactory(BaseFactory):
    ...
    current_balance = fuzzy.FuzzyDecimal(0, 50000, 2)
    customer_type = factory.Iterator(["wholesale", "retail", "distributor"])
    is_active = True
    created_at = factory.LazyFunction(datetime.utcnow)
...
class SupplierFactory(BaseFactory):
    ...
    payment_terms = factory.Iterator(["Net 30", "Net 60", "COD", "50% Advance"])
    is_active = True
    created_at = factory.LazyFunction(datetime.utcnow)
