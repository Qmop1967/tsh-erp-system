...
echo "🚀 TSH Salesperson App - iPhone Launch Script"
echo "=============================================="

# Navigate to the salesperson app directory
...
if [ -z "$IPHONE_DEVICE" ]; then
    ...
    echo "🚀 Launching TSH Salesperson App on your iPhone..."
    ...
    if [ -n "$DEVICE_ID" ]; then
        ...
        if ! flutter run -d "$DEVICE_ID" --release; then
            ...
            echo "Please check the iOS project configuration in Xcode."
            echo ""
            echo "Try opening: ios/Runner.xcworkspace"
            echo "And configure Signing & Capabilities"
            echo ""
            echo "------------------------------------------------------"
        fi
    else
